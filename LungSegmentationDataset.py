...
class LungSegDataset(Dataset):  # inherit from torch.utils.data.Dataset
    ...
    def __init__(
        self,
        root_dir=os.path.join(os.getcwd(), "data/Lung Segmentation"),
        split="train",
        transforms=None,
        shuffle=True,
    ):
        ...
        self.train_image_idx = sorted(
            [int(fName.split("_")[1]) for fName in self.train_image_file]
        )

        self.eval_image_file = [fName for fName in image_file if "MCUCXR" in fName]
        self.eval_image_idx = sorted(
            [int(fName.split("_")[1]) for fName in self.eval_image_file]
        )
        ...
        self.train_mask_idx = sorted(
            [int(fName.split("_")[1]) for fName in self.train_mask_file]
        )

        self.eval_mask_file = [fName for fName in mask_file if "MCUCXR" in fName]
        self.eval_mask_idx = sorted(
            [int(fName.split("_")[1]) for fName in self.eval_mask_file]
        )
        ...
        self.train_idx = [
            idx for idx in self.train_image_idx if idx in self.train_mask_idx
        ]
        self.eval_idx = [
            idx for idx in self.eval_image_idx if idx in self.eval_mask_idx
        ]
        self.val_idx = self.eval_idx[: int(0.5 * len(self.eval_idx))]
        self.test_idx = self.eval_idx[int(0.5 * len(self.eval_idx)) :]

        self.data_file = {
            "train": {"image": self.train_image_file, "mask": self.train_mask_file},
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(img_path).convert(
            "LA"
        )  # open as PIL Image and set Channel = 1
...
if __name__ == "__main__":
    ...
    from my_transforms import GrayScale, Resize, ToTensor

    # set img size
    img_size = 512

    scale = Resize(512)
    composed = transforms.Compose([Resize(600), GrayScale()])

...
def train_and_validate(
    ...
    for epoch in range(epochs):
        ...
        for phase in ["train", "val"]:
            ...
            if phase == "val" and epoch_acc > best_acc:
                ...
                torch.save(
                    ...
                    os.path.join(
                        os.getcwd(),
                        "checkpoint/best_checkpoint[epoch_{}].pt".format(epoch + 1),
                    ),
                )
        ...
        print(
            "One Epoch Complete in {:.0f}m {:.0f}s".format(
...
def get_args():
    parser = argparse.ArgumentParser(
        description="U-Net for Lung Segmentation",
    ...
    parser.add_argument(
        "--n_workers", type=int, default=4, help="The number of workers for dataloader"
    )
    ...
    parser.add_argument(
        "--load_model", type=str, default=None, help=".pth file path to load model"
    )
...
def main():
    ...
    eval_transforms = transforms.Compose([GrayScale(), Resize(img_size), ToTensor()])
    ...
    dataloader = {
        ...
        "val": DataLoader(
            dataset=val_dataset, batch_size=args.batch_size, num_workers=args.n_workers
        ),
        "test": DataLoader(
            dataset=test_dataset, batch_size=args.batch_size, num_workers=args.n_workers
        ),
    }
